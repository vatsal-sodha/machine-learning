...
X = [ones(m, 1) X]; %adding one's column to X
a1=X;
z2=X*Theta1';
a2=sigmoid(z2);
a2=[ones(m,1) a2]; %adding bias unit
z3=a2*Theta2';
a3=sigmoid(z3); %forward Propogation and got hypothesis value
for k = 1:num_labels %summing over 1 to K as per definition of cost function
    yk = y == k;
    hthetak = a3(:, k);
